 ...
 * 1) EXPO_PUBLIC_API_BASE o EXPO_PUBLIC_API (p. ej. http://206.189.69.68/condos/api)
...
function resolveBase(): string {
  // 1) Variables completas (recomendado)
  const envFull =
    process.env.EXPO_PUBLIC_API_BASE?.trim() ||
    process.env.EXPO_PUBLIC_API?.trim();
  if (envFull) return envFull.replace(/\/+$/, "");

  // 2) URL de ngrok sin path (opcional; ya casi no lo usas)
...
export async function api(
  ...
  // ❌ IMPORTANTE: ya NO metemos ngrok-skip-browser-warning aquí
  ...
  if (!ct.includes("application/json")) {
    const txt = await res.text().catch(() => "");
    throw new Error(
      `Unexpected response (status ${res.status}): ${ct}\n${txt?.slice(0, 300)}`
    );
  }
  return res.json();
}
